...
export const ModelDynamicTable= ()=>  {
    const getColor = (elem:number) => {
        switch (true) {
            case elem >= 100: return '#34C759';
            case (elem >= 90 && elem < 100): return '#FF9500';
                ...
                {metrics.map((metric) => (
                    <>
                        <TableRow className="border" key={metric.id}>
                            <TableCell className="font-medium border">{metric.name}</TableCell>
                        </TableRow>
                        <TableRow className={"p-5px"} key={metric.id}>
                            <TableCell style={{ paddingLeft: '15px'}} className="border">факт 2024</TableCell>
                        ...
                        <TableRow key={metric.id}>
                            <TableCell style={{ paddingLeft: '15px'}} className="border">цель 2024</TableCell>
                        ...
                        <TableRow key={metric.id}>
                            <TableCell style={{ paddingLeft: '15px'}} className="border">выполнение</TableCell>
                            <TableCell className="border">{metric.compl.dim}</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.jan)}}>{metric.compl.jan}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.feb)}}>{metric.compl.feb}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.mar)}}>{metric.compl.mar}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.apr)}}>{metric.compl.apr}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.may)}}>{metric.compl.may}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.jun)}}>{metric.compl.jun}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.jul)}}>{metric.compl.jul}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.aug)}}>{metric.compl.aug}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.sep)}}>{metric.compl.sep}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.oct)}}>{metric.compl.oct}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.nov)}}>{metric.compl.nov}%</TableCell>
                            <TableCell className="border" style={{color: getColor(metric.compl.dec)}}>{metric.compl.dec}%</TableCell>
                        </TableRow>
                    </>
                ))}
            </TableBody>
