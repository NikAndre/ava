...
export function ModelDataTable() {
  const columns: ColumnDef<VariablesType>[] = [
          ...
          {row.getCanExpand() ? (
            <button
              {...{
                ...
                style: { cursor: 'pointer', background: '#ffffff', marginRight: '5px' },
              }}
            >
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="lowercase">{row.getValue("factLY")}</div>,
      footer: props => props.column.id,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="lowercase">{row.getValue("factVsTargetCY")}</div>,
      footer: props => props.column.id,
    },
    {
                ...
                <>
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id} className={`border` + " " + style['cell']}>
                        {flexRender(
