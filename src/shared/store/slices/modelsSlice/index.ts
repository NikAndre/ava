  ...
  reducers: {
    ...
    setActiveModel: (state, action) => {
      if (action.payload.id) {
        const model = [...state.modelsList].find(elem => elem.id === action.payload.id)
        state.activeModel = model
      }
    },
    addModelToCheckedList: (state, action) => {
      if (action.payload.model.id) {
        const isInList = [...state.checkedModelsList].find(elem => elem.id === action.payload.model.id)
    ...
    removeModelFromCheckedList: (state, action) => {
      const newCheckedArray = state.checkedModelsList?.filter(elem => elem.id !== action.payload.id)
    ...
    setActiveTab: (state,action) => {
      const isInChecked =  state.checkedModelsList?.find(elem => elem.name !== action.payload.name)
    ...
    addMetricToCheckedList: (state, action) => {
      if (action.payload.id) {
        const metric = [...state.checkedMetricsList.metrics].filter(elem => elem.id === action.payload.id)
