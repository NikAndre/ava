...
import {clearActiveVariable, setActiveVariable, setActiveTab, removeVariableFromCheckedList} from '@/shared/store/slices/variablesSlice'
import { SquareX } from 'lucide-react'

const AdminPage:FC = () => {
  const activeVariable = useSelector((store) => store.variables.activeVariable)
  const checkedVariables = useSelector((store) => store.variables.checkedVariablesList)
      ...
      <main className={styles['content_wrapper']} style={{ display:'flex', flexDirection:'column', gap: '20px', padding: '30px 34px'}}>
        ...
        <Tabs defaultValue={'all'} value={activeTab}  className="w-[400px]">
          <TabsList style={{gap:'10px'}}>
            ...
            }} value="all">Все переменные</TabsTrigger>
